...
# URL del servi  dozr con el modelo BLIP
MODEL_SERVER_URL = "http://192.168.100.159:8000"

# --- CAMBIO IMPORTANTE PARA RASPBERRY PI ---
PUERTO = '/dev/rfcomm0'  
# -------------------------------------------
...
def esp32_connection_thread():
    ...
    print(f"\n--- Iniciando Bucle de Conexión en RASPBERRY ({PUERTO}) ---")
    ...
    while ESP32_ENABLED:
        try:
            ...
            ser = serial.Serial(PUERTO, ESP32_BAUDRATE, timeout=1)
            
            # --- BLOQUE DE CONEXIÓN EXITOSA ---
            print("\n" + "="*40)
            print(f"¡CONEXIÓN EXITOSA! Puerto {PUERTO} abierto.")
...
async def health_check():
    ...
    try:
        ...
        return {
            ...
            "esp32_port": PUERTO if ESP32_ENABLED else None
        }
    except Exception as e:
        return {
            "gateway_status": "healthy",
